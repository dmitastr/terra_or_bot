...
FILTER_QUERY_TEMPLATE = 'SELECT * FROM {table_name} {where_clause} {order_by_clause} {limit_clause}'
...
class YDataBase:
    ...
    def execute_query(self, query) -> list[dict]:
        ...
        try:
            if result_sets:
                result_parsed = [self.parse_row(row)
    ...
    def create_select_query(
        ...
        where_clause = 'WHERE ' + \
            ' AND '.join(where_clauses) if field_filter else ''
